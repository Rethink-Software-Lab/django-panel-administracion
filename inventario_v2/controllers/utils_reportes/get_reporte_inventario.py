...
from django.db.models import F, Count, DecimalField, Value
from django.db.models.base import Coalesce
from django.shortcuts import get_object_or_404
from pydantic import Field

from inventario.models import AreaVenta, ProductoInfo, Productos_Cafeteria
...
def get_reporte(area: str, categoria: str) -> GetReporte:
    def search_and_format(filters: Optional[dict[str, bool | str]] = None) -> list[Productos]:
        ...
        productos = (
            ...
            .values("id", "descripcion", "cantidad")
        )
    ...
    match area:
        case "general":
            ...
            producto_info_cafeteria = (
                Productos_Cafeteria.objects.annotate(
                    total_cantidad=Coalesce(F("inventario_area__cantidad"), Value(0, output_field=DecimalField())) +
                                Coalesce(F("inventario_almacen__cantidad"), Value(0, output_field=DecimalField())),
                )
        ...
        case "cafeteria":
            ...
            producto_info = Productos_Cafeteria.objects.filter(
